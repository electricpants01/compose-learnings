...
@Composable
fun AdaptiveVerticalGridScreen(
    modifier: Modifier,
) {
    val itemList = (0..100).map { "Item $it" }
    val columns by remember {
        mutableStateOf(GridCells.Adaptive(180.dp))
    }
...
@Composable
fun AdaptiveHorizontalGridScreen(
    modifier: Modifier,
) {
    val itemList = (0..100).map { "Item $it" }
    val rows by remember {
        mutableStateOf(GridCells.Adaptive(130.dp))
    }
...
private fun PhonePreview() {
    Column {
        AdaptiveVerticalGridScreen(modifier = Modifier.weight(1f).padding(4.dp))
        AdaptiveHorizontalGridScreen(modifier = Modifier.weight(1f).padding(4.dp))
    }
}
